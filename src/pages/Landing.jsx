    ...
    <div style={{ fontFamily: "'Inter', sans-serif", background: '#0a0f1e', minHeight: '100vh', color: '#fff' }}>
      ...
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1.2rem 2.5rem', borderBottom: '1px solid #1e2d4a' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          <button onClick={() => setMode('login')} style={{ background: 'transparent', border: '1px solid #1e2d4a', color: '#aab', padding: '0.5rem 1.2rem', borderRadius: 8, cursor: 'pointer', fontSize: '0.9rem' }}>Iniciar sesión</button>
          <button onClick={() => setMode('register')} style={{ background: 'linear-gradient(135deg,#00d4aa,#0066ff)', border: 'none', color: '#fff', padding: '0.5rem 1.4rem', borderRadius: 8, cursor: 'pointer', fontWeight: 600, fontSize: '0.9rem' }}>Comenzar gratis</button>
        </div>
      </nav>
      ...
      <div style={{ textAlign: 'center', padding: '5rem 2rem 3rem' }}>
        ...
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => setMode('register')} style={{ background: 'linear-gradient(135deg,#00d4aa,#0066ff)', border: 'none', color: '#fff', padding: '0.9rem 2.5rem', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: '1.05rem' }}>
            Crear cuenta gratis →
          </button>
          <button onClick={() => setMode('login')} style={{ background: '#0d2040', border: '1px solid #1a3a6a', color: '#ccd', padding: '0.9rem 2rem', borderRadius: 10, cursor: 'pointer', fontSize: '1.05rem' }}>
            Ya tengo cuenta
          </button>
        </div>
      </div>
      ...
      <div style={{ textAlign: 'center', padding: '4rem 2rem', background: '#060d1a' }}>
        ...
        <button onClick={() => setMode('register')} style={{ background: 'linear-gradient(135deg,#00d4aa,#0066ff)', border: 'none', color: '#fff', padding: '1rem 3rem', borderRadius: 10, cursor: 'pointer', fontWeight: 700, fontSize: '1.1rem' }}>
          Crear cuenta gratis
        </button>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '1rem' }}>
          <div style={{ background: '#0d1a30', border: '1px solid #1e2d4a', borderRadius: 16, padding: '2.5rem', width: '100%', maxWidth: 420 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
              <h2 style={{ margin: 0, fontSize: '1.4rem' }}>{mode === 'login' ? 'Iniciar sesión' : 'Crear cuenta'}</h2>
              <button onClick={() => setMode(null)} style={{ background: 'none', border: 'none', color: '#aab', fontSize: '1.5rem', cursor: 'pointer' }}>×</button>
            </div>
            <form onSubmit={submit}>
              ...
              {error && <div style={{ background: '#2d0a0a', border: '1px solid #5a1a1a', borderRadius: 8, padding: '0.75rem', color: '#ff6b6b', fontSize: '0.85rem', marginBottom: '1rem' }}>{error}</div>}
